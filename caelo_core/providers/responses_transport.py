"""Neutralny transport dla API zgodnego z OpenAI Responses.

Moduł nie wybiera dostawcy, modelu, narzędzi ani źródła poświadczeń. Adapter
providera przekazuje jawny adres bazowy, klucz, politykę retencji i opcjonalny
przelicznik kosztu. Dzięki temu parser SSE i pętla function callingu mogą być
współdzielone przez xAI i OpenAI bez przenikania ich polityk.
"""
...
def stream_response(
    ...
    *,
    ...
    cost_resolver: Optional[Callable[[dict], Optional[float]]] = None,
) -> dict:
    """Strumieniuj odpowiedź z API zgodnego z OpenAI Responses.
    ...
    real_cost = cost_resolver(usage) if cost_resolver is not None else None
