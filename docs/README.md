...
| Dokument | Co zawiera | Status |
|---|---|---|
| [`plans/PLAN_OTWARTE.md`](plans/PLAN_OTWARTE.md) | **★ Zbiorczy plan wszystkich niezrealizowanych punktów** (2026-06-17) — Faza B ✅ (release `v0.1.0`); weryfikacja LIVE A–I+K ✅, **zostaje tylko J** (mac/Linux); otwarte: TOP-10 (TOP7–10), motywy inżynierskie 4.1, spike B0, ROAD-4.2 (inni dostawcy LLM), per-hunk diff (M13-F5). Jedno źródło prawdy „co zostało". | 🔧 w toku |
...
| [`plans/PLAN_NAPRAWY_4.md`](plans/PLAN_NAPRAWY_4.md) | **Runda napraw #4** (84 znaleziska z analizy 2026-06-10) — Fazy **A/D/E/F ✅** (P1 + P2/P3 współbieżność/odzysk/limity + frontend). Faza B (publikacja) ✅, C (LIVE) 🟡 tylko J. **Otwarte:** G (TOP7–10) + motywy 4.1. | 🟡 A/D/E/F ✅ |
| [`plans/PLAN_WERYFIKACJI_LIVE.md`](plans/PLAN_WERYFIKACJI_LIVE.md) | **Runbook weryfikacji LIVE** (sandbox blokuje xAI/exec — robi user): **A–I + K ✅** (w tym G4 remote-MCP, G7 ACP, K1 terminal); **otwarte tylko J** (mac/Linux). Tabela wyników na górze. | 🟡 (tylko J) |
| [`plans/FAZA_C_PRZYKLADY.md`](plans/FAZA_C_PRZYKLADY.md) | **Cookbook Fazy C** — gotowe przykłady „co zrobić" do weryfikacji LIVE: konkretne prompty, komendy, configi (`.caelo/permissions.json`, `lsp.json`) + kryteria „✅ zaliczone". Zostało już tylko **J** (mac/Linux). | 🔧 tylko J |

### Archiwum — zrealizowane rundy napraw

| Dokument | Co zawiera | Status |
|---|---|---|
| [`plans/zrealizowane/ANALIZA_PROGRAMU_2026-06-10.md`](plans/zrealizowane/ANALIZA_PROGRAMU_2026-06-10.md) | **Gruntowny przegląd programu (2026-06-10)** — rewizja wieloagentowa 5 obszarów + testy, 10×P1, katalog P2/P3, **porównanie z konkurencją (TOP-10)**. Źródło `PLAN_NAPRAWY_4.md`. | 📋 analiza |
| [`plans/zrealizowane/PLAN_FAZA_B_RUNBOOK.md`](plans/zrealizowane/PLAN_FAZA_B_RUNBOOK.md) | **Runbook publikacji (Faza B)** — remote (`AuraVixStudio/caelo`) → CI → gitleaks → podpisany release (SimplySign) z auto-update. **Kroki 0–6 ✅ (2026-06-17, release `v0.1.0`)**; pozostaje tylko public repo (→ auto-update end-user). | ✅ (poza public) |
