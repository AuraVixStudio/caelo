...
- **REST** (Bearer, poza `/health`): `GET /health` · `GET /whoami` · `GET /auth/status` · `POST /auth/login`
  ...
  · `GET /agent/checkpoints` · `POST /agent/undo` · `GET`/`PUT /agent/grok-md` *(M13)*.
...
same dodania, plik binarny → znacznik „binary"); **tryby agenta** jak „Mode" w Claude Code
(`ask`/`accept-edits`/`plan`/`bypass` — plan = tylko READONLY i propozycja planu, accept-edits = auto
write/edit, bypass = auto wszystko); **checkpointy + undo** bez zależności od gita (kopia plików do
`.grok/checkpoints/<sid>/` + manifest atomowy; „Undo to checkpoint"/„Undo all"; `run_command` →
„undo częściowy"); **`GROK.md`** auto-pamięć projektu (workspace + globalny `DATA_DIR`) w system
