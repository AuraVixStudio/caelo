...
`grok-imagine-image` · wideo `grok-imagine-video-1.5` · głos `grok-voice-latest` ·
embeddingi `embedding-beta-3-small` (⛔ xAI 404 — B8 uśpione). Wizja wymaga rodziny **grok-4**.
...
- [ ] **D3 — STT stream (partiale).**  🔄 PONOWNIE OTWARTE 2026-06-23 (przegląd docs xAI): nasza próba z 2026-06-19 wysłała **ZŁY format ramek** — `error: unknown variant input_audio_buffer.append, expected audio.done`. Odłożyliśmy to jako „nieudokumentowane". **TERAZ PROTOKÓŁ JEST UDOKUMENTOWANY** ([Speech to Text | xAI Docs](https://docs.x.ai/developers/model-capabilities/audio/speech-to-text)) i błąd mówił prawdę: klient ma streamować **SUROWE AUDIO jako BINARNE ramki WS** (NIE JSON `input_audio_buffer.append`), zakończyć `{"type":"audio.done"}` → `transcript.done`; config przez query-params (`sample_rate` ∈ 8000/16000/22050/24000/44100/48000, `encoding` ∈ pcm-s16le/mulaw/alaw). xAI dodało też serwerowy **„Smart Turn"** (detekcja końca wypowiedzi).
  - *Status:* **NIE ślepa uliczka — follow-up implementacyjny.** Przepiąć `/voice/stt/stream` + `lib/converse.ts` na binarne ramki PCM + `audio.done` (zamiast `input_audio_buffer.append`). Most Live/`/v1/realtime` nietknięty (realtime faktycznie używa `input_audio_buffer.append`). Do tego czasu Talk (D4) działa na batch-STT + VAD (sprawne).
...
- [x] **D4 — Talk (pipeline converse) + batch-STT + VAD + barge-in.**  ✅ 2026-06-19 (fix `497cbb2`). Po werdykcie D3 Talk używa **batch `/voice/stt`** (jak dyktowanie D2) z lokalnym **VAD** (AnalyserNode RMS, auto-stop na ~1,5 s ciszy): mów → cisza → transkrypt → `/voice/converse` (Responses M10 + live search + historia M9) → odpowiedź tekst+głos. Barge-in zachowany (mowa w trakcie TTS przerywa). **ZWERYFIKOWANE NA ŻYWO 2026-06-19:** „Jaka jest stolica Francji?" → VAD auto-stop → batch-STT (PL) → „Paryż." (tekst+głos, TTS 6 chars) → Listening; badge sumuje STT 5s · TTS 6 chars.
  ...
  - *Do dobrania w razie potrzeby:* progi VAD `SILENCE_MS=1500` / `SPEECH_RMS=0.02` w `converse.ts`.
...
- [x] **D6 — Read-aloud + D7 koszt sesji.**  ✅ 2026-06-19. D6: read-aloud w czacie używa głosu z ustawień (potwierdzone przy D1, po zmianie głosu w Settings). D7: badge w module Voice sumuje audio per sesja przez wszystkie tryby (np. `STT 5s · TTS 6 chars · ~$0.0002` po turze Talk; rośnie z każdą operacją Speak/Transcribe/Talk/Live). ⚠️ koszt TTS = strojalny szacunek — do weryfikacji na fakturze.
...
- [x] **H4 — web_fetch (M19-B13).**  ✅ 2026-06-19 (`CAELO_WEB_FETCH=1` + `CAELO_WEB_FETCH_DOMAINS=example.com`). Wszystkie 3 zachowania potwierdzone na żywo: (1) pozytyw — `https://example.com` pobrane („Example Domain"); (2) SSRF-guard — `https://127.0.0.1/` → `refused host (loopback/private not allowed)`; (3) twarda allowlista — `https://www.google.com` → `not in the web_fetch allowlist`. Bonus: „INTEROP-OK: AGENTS.md was loaded" (G5 interop też OK).
  - *Decyzja:* **zostaw jako udokumentowany OPT-IN** — egress sieciowy nie włączamy domyślnie; bezpieczne (https-only + SSRF + allowlista + cap). Bez flagi narzędzie ukryte.

- [x] **H5 — git worktree (M19-B12).**  ✅ 2026-06-19 (`$env:CAELO_GIT_WORKTREE="1"`, projekt testowy = repo git). Delegacja do `implementer` → `git worktree list` (na pauzie „Review merge") pokazał REALNY worktree subagenta: `...\grok_desktop_app\worktrees\run-23468-dc6db0f6-4\sa4  b964d40 (detached HEAD)` poza `[master]`. Po Accept/Discard sprzątany. Potwierdza wiązanie env→config→team→`create_worktree(use_git=True)` (selfcheck `B12: repo + use_git creates a git worktree`).
  - *Decyzja:* **opt-in, zostaw** (działa też poza repo jako copy-fallback, zweryfikowany w F2).

- [x] **H6 — auto-compact (M19-B10).**  ✅ (selfcheck) 2026-06-19. Live niepraktyczne — próg `AGENT_COMPACT_THRESHOLD_CHARS=48000` (niewystawiony przez env) wymagałby ~48k-znakowej sesji. Mechanizm pokryty `agent_selfcheck` (próg 300: zwijanie na granicy `user`, nagłówek `COMPACT_SUMMARY_HEADER`, skrócenie historii, balans tool_call↔tool). *Decyzja:* **opt-in, zostaw** (off-by-default rozsądny; mechanizm zweryfikowany testem).
