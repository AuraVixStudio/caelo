...
- [ ] **D3 — STT stream (partiale).** Tryb live STT (`/voice/stt/stream`).
  ...
  - *Bug naprawiony (live, blokował D3/D4/D5):* CSP `script-src` nie miał `blob:` → AudioWorklet (`MicCapture`, PCM16) nie wstawał → „Audio capture is unavailable in this environment." Dodano `blob:` do `script-src` w `index.html` (commit `a02e67f`) + test `desktop/test/csp.test.ts`. **Wymaga pełnego restartu `npm run dev` (meta CSP czytana przy ładowaniu dokumentu).**
