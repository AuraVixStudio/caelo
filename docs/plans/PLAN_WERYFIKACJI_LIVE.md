...
| D | Głos | P2 | 🟡 | 2026-06-19 | **D1 (TTS) ✅** — 5 głosów (Eve/Ara/Rex/Sal/Leo) + EN/PL, read-aloud w czacie + tryb Speak, badge kosztu rośnie zgodnie ze stawką ($0.015/1k). **1 bug UX naprawiony**: potwierdzenia zapisu Settings szły w banner na górze (poza widokiem) → toast (`17c2caa`, +test regresyjny). ⚠️ `TTS_COST_PER_1K_CHARS` to SZACUNEK — do weryfikacji na fakturze. Uwaga (nie-bug): tryb Speak pokazuje tylko ostatnią generację (`ttsAudio` single-state); pliki na dysku + „Open file". **Zostają D2–D7 — user ma problem z mikrofonem (blokada).** |
