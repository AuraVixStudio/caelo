...
Weryfikuje `grok_core.genjobs.GenJobManager` na temp `HistoryStore` z atrapą
...
Wzorzec jak pozostałe skrypty w `grok_core/tools/`: lista (nazwa, ok), kod wyjścia
...
def _store(d: str):
    from grok_core.history_store import HistoryStore
...
def _unit_lifecycle(checks: list) -> None:
    ...
    from grok_core.genjobs import GenJobManager, estimate_cost
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            try:
                ...
                from grok_core.genjobs import DONE
...
def _unit_cancel(checks: list) -> None:
    ...
    from grok_core.genjobs import GenJobCancelled, GenJobManager
...
def _unit_queue_limit(checks: list) -> None:
    ...
    from grok_core.genjobs import GenJobManager, GenJobQueueFull
...
def _unit_backend_image_executor(checks: list) -> None:
    ...
    import grok_core.history_store as HS
    import grok_core.state as state_mod
    from grok_core.genjobs import GenJob
...
def _unit_backend_video_executor(checks: list) -> None:
    ...
    import grok_core.history_store as HS
    import grok_core.state as state_mod
    from grok_core.genjobs import GenJob, GenJobCancelled
...
def _unit_route_validation(checks: list) -> None:
    ...
    from grok_core.routes.genjobs import ImageJobReq, VideoJobReq
...
def main() -> int:
    ...
    logging.getLogger("grok.genjobs").setLevel(logging.CRITICAL)
