"""Punkt wejścia sidecara: ``python -m grok_core``.

Przepływ handshake (uzgodniony z procesem głównym Electron):
- token sesji: z env ``GROK_CORE_TOKEN`` (ustawiany przez Electron) lub — przy
  uruchomieniu samodzielnym — generowany losowo,
- port: z env ``GROK_CORE_PORT`` lub pierwszy wolny port na 127.0.0.1,
...
from grok_core.server import APP_VERSION, create_app

HANDSHAKE_PREFIX = "__GROK_CORE_READY__"
...
def main() -> None:
    ...
    token = os.environ.get("GROK_CORE_TOKEN") or secrets.token_urlsafe(32)
    port = int(os.environ.get("GROK_CORE_PORT") or _free_port())
