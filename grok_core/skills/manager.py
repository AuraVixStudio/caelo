...
Źródła: wbudowane (`builtin/`, pakowane) + użytkownika (`config.SKILLS_DIR`). Skill
o tej samej nazwie w katalogu usera NADPISUJE wbudowany. Stan „enabled" trzymany w
`SKILLS_DIR/_state.json`. Włączone skille → `injected_text()` (cap rozmiaru), wstrzykiwane
do system promptu agenta. Brak/uszkodzony/za duży SKILL.md tolerowany (skip-with-log).
"""
...
from grok_core.markdown_meta import parse_frontmatter
...
class SkillManager:
    ...
    # --- odkrywanie ---
    def _read_skill(self, folder: Path, *, builtin: bool) -> Optional[dict]:
    ...
    def _scan(self, base: Path, *, builtin: bool) -> dict[str, dict]:
        out: dict[str, dict] = {}
        try:
            if not base.is_dir():
                return out
            for folder in sorted(base.iterdir()):
                if not folder.is_dir() or folder.name.startswith((".", "_")):
                    continue
                sk = self._read_skill(folder, builtin=builtin)
                if sk:
                    out[sk["id"]] = sk
        except Exception:  # noqa: BLE001
    ...
    def _all(self) -> dict[str, dict]:
        merged = self._scan(BUILTIN_DIR, builtin=True)
        merged.update(self._scan(self._user_dir, builtin=False))  # user nadpisuje builtin
    ...
    def create_skill(self, sid: str, *, template: str = "blank",
        ...
        with self._lock:
            ...
            sk = self._read_skill(folder, builtin=False)
        if sk is None:
    ...
    def delete_skill(self, sid: str) -> bool:
        with self._lock:
            sk = self._all().get(sid)
            if sk is None or sk.get("builtin"):
                return False  # wbudowanych nie usuwamy
            folder = (self._user_dir / sid)
            try:
